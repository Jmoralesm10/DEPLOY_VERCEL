...
import { DatosMunicipioService } from 'src/app/services/datosmunicipio.service';
...
export class AppFormComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, public loginService: LoginService, private ToastrService: ToastrService, public rescateService: RescateService, public municipioService: MunicipioService, public datosmunicipioService: DatosMunicipioService, public catalogoService: CatalogoService) {
    ...
    this.fgFormOperador = this.fb.group({
      ...
      Nombre_Solicitante: [''],
      Cod_TipoAviso: [0],
      Cod_Compania_Salida: [0],
      Fecha_Salida: [''],
      Cod_Compania_Entrada: [0],
      Fecha_Entrada: [''],
    ...
    this.fgFormOperador?.get('Area')?.valueChanges.subscribe((value) => {
      if (value === '') {
        ...
        this.fgFormOperador?.get('Zona')?.setValue(''); // Limpiar el valor del campo 'zona'
        this.fgFormOperador?.get('Cod_Lugar')?.setValue(''); // Limpiar el valor del campo 'zona'
  ...
  ngOnInit(): void {
    ...
    // Muestra el toast de carga al iniciar el componente
    this.ToastrService.info('Cargando...', '', {
      disableTimeOut: true, // El toast no se oculta automáticamente
      closeButton: false, // No muestra el botón de cierre
      positionClass: 'toast-bottom-right' // Posiciona el toast en la esquina inferior derecha
    });


    // Simula una operación de inicialización que tarda unos segundos
    setTimeout(() => {
      // Cambia loading a false cuando la inicialización ha terminado
      this.loading = false;
      // Oculta el toast cuando la inicialización ha terminado
  ...
  submit() {
    ...
    if (this.fgFormOperador.invalid) {
